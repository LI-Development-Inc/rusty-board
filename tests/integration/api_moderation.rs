...
impl FlagRepository for NopFlag {
    ...
    async fn resolve(&self, _: FlagId, _: FlagResolution, _: Option<UserId>)
        -> Result<(), DomainError>
    {
        Ok(())
    }
...
async fn toggle_sticky_returns_204() {
    let thread_id = Uuid::new_v4();
    let resp = mod_app()
        .oneshot(with_mod_user(plain_post(&format!("/mod/threads/{thread_id}/sticky"))))
        .await
        .unwrap();
    assert_eq!(resp.status(), StatusCode::NO_CONTENT);
}
...
async fn toggle_close_returns_204() {
    let thread_id = Uuid::new_v4();
    let resp = mod_app()
        .oneshot(with_mod_user(plain_post(&format!("/mod/threads/{thread_id}/close"))))
        .await
        .unwrap();
    assert_eq!(resp.status(), StatusCode::NO_CONTENT);
}
...
async fn create_flag_returns_201_without_auth() {
    ...
    let resp = mod_app()
        .oneshot(json_post(
            ...
            r#"{"post_id":"00000000-0000-0000-0000-000000000001","reason":"off-topic content"}"#,
        ))
        .await
        .unwrap();
