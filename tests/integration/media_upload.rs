...
impl PostRepository for OkPostRepo {
    ...
    async fn save(&self, p: &Post) -> Result<PostId, DomainError> { Ok(p.id) }
...
fn inject_ctx(mut req: Request<Body>, board_id: BoardId) -> Request<Body> {
    ...
    config.allowed_mimes       = vec!["image/jpeg".to_owned(), "image/png".to_owned()];
    config.max_files            = 3;
...
#[tokio::test]
async fn post_with_jpeg_attachment_returns_201() {
    ...
    let svc = Arc::new(PostService::new(
        ...
        storage_clone, AllowAllRateLimiter, StubProcessor,
    ));
...
async fn post_with_disallowed_mime_is_rejected() {
    ...
    let svc = Arc::new(PostService::new(
        OkPostRepo, OkThreadRepo, NoBanRepo,
        TrackingStorage::default(), AllowAllRateLimiter, StubProcessor,
    ));
    let app = post_routes(svc);
