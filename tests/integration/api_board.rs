...
use chrono::Utc;
use domains::{errors::DomainError, models::*, ports::BoardRepository};
...
async fn list_boards_returns_200_with_page() {
    let app = board_public_routes(Arc::new(OkBoardRepo::for_slug("tech")));
...
async fn list_boards_returns_empty_page_when_no_boards() {
    let app = board_public_routes(Arc::new(NotFoundBoardRepo));
    let resp = app.oneshot(get("/boards")).await.unwrap();
...
async fn get_board_by_slug_returns_200_for_existing() {
    let app = board_public_routes(Arc::new(OkBoardRepo::for_slug("b")));
